...
- New algorithms go in every profile's `hashrates` block, the `requiredAlgos` list in `internal/cost/hardware_test.go`, and the `--algo` row of the README table. The `--algo` help string and the unknown-algorithm error are generated from the YAML. **A tunable algorithm also needs a `cost.TuningFor` case and a scaling branch in `CalculateHashRate`** — without the first, `--cost` and `--memory` are rejected for it; without the second, the work factor is accepted and then ignored. Both fail closed (a visible error, or the un-tuned baseline), but only the `TuningFor` case makes the flags work.
