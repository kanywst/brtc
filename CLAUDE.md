...
- **bcrypt baseline is at cost = 5**, not the cobra default of `cost = 10`. The factor in `CalculateHashRate` is `2^(workFactor - 5)`. If you change the baseline you must update both the comment and the math.
